...
public class DoorSpecialRelease extends Entity{
	
	private int x;
	private int y;
	private int pickupDist;
	private Player player;
	private int color = Colors.get(-1, 000, 500, -1);
	
	public boolean destroyed = false;
	
	public DoorSpecialRelease(Level level, int x, int y, int pickupDist, Player player) {
		super(level);
		this.x = x;
		this.y = y;
		...
		if (!destroyed) {
			...
			if (playerDist < pickupDist) {
				for (Entity entity : level.entities) {
					if (entity.getClass() == DoorSpecial.class) {
